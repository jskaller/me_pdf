...
class OrchestratorGuardedFormWidgetPolicyTests(unittest.TestCase):
    ...
    def test_orchestrator_runs_guarded_apply_validation_and_acceptance(self) -> None:
        ...
        self.assertIn("evaluate_guarded_acceptance", self.orchestrator_text)
        self.assertIn("guarded_form_widget_acceptance", self.orchestrator_text)
        self.assertIn("apply_guarded_form_widget_overall", self.orchestrator_text)

    def test_candidate_promotion_remains_guarded_not_default(self) -> None:
        self.assertIn('guarded_form_widget_acceptance_decision.get("promote_candidate_to_final")', self.orchestrator_text)
        self.assertIn("FINAL_PDF_GUARDED_CANDIDATE", self.orchestrator_text)
        self.assertNotIn('FINAL_PDF = paths["candidate_pdf"]', self.orchestrator_text)
        self.assertNotIn("FINAL_PDF = guarded_form_widget_apply_report", self.orchestrator_text)
