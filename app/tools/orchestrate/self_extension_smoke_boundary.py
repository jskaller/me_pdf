...
This wrapper is used for H13/H13S/H13T/H13U evidence-only validation. It runs
remediate.py with self-extension configured, blocks source/rule-map/adoption
drift, persists fail-closed artifacts if the child command fails early, and now
surfaces a fixture preflight verdict before any retry-loop proof is claimed.
"""
...
def source_snapshot(app_dir: Path, workspace: Path | None = None) -> dict:
    ...
    for path in candidates:
        ...
        snapshot[rel] = hashlib.sha256(path.read_bytes()).hexdigest() if path.exists() else None
    return snapshot
...
def source_mutation_actions(before: dict, app_dir: Path, workspace: Path | None = None) -> list[dict]:
    ...
    for rel in sorted(set(before) | set(after)):
        if before.get(rel) == after.get(rel) or not is_prohibited_source_path(rel):
            continue
...
def _actual_rule_ids_from_preflight(preflight: dict) -> list[str]:
    actual = preflight.get("actual_target_rule_id")
    return [str(actual)] if actual else []


def surface_smoke_boundary(job_dir: Path, *, expected_target_rule_id: str | None, self_extension_configured: bool = True, blocked_actions: Iterable[dict] | None = None, transport_unavailable: bool = False, child_result: dict | None = None, rule_map_path: Path | None = None, fixture: str | None = None) -> dict:
    job_dir = Path(job_dir)
    ...
    actual_rules = _actual_rule_ids_from_preflight(preflight)
    ...
    if target_check["result"] == "NO_RESIDUAL_GAP":
        actions.append(blocked_action("fixture_preflight_no_target", reason="no_residual_rule_selected"))
    if transport_unavailable:
    ...
    for payload, path in ((existing_outcome, outcome_path), (existing_status, status_path)):
        ...
        payload["fixture_preflight"] = {
            ...
            "artifact_path": preflight.get("artifact_path"),
        }
...
def run_wrapper(args: argparse.Namespace) -> int:
    ...
    summary = surface_smoke_boundary(
        ...
        rule_map_path=app_dir / "tools" / "audit" / "rule_repair_map.json",
        fixture=f"{args.ticket}/{args.basename}",
    )
    print(json.dumps(summary, indent=2, sort_keys=True))
    if proc.returncode != 0 or actions or not summary.get("fixture_preflight", {}).get("self_extension_would_run") or summary["self_extension"].get("result") == "NOT_RUN":
        return 1
    return proc.returncode


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run remediation in evidence-only self-extension smoke mode")
